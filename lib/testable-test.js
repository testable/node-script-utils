...
class TestSteps {
  ...
  constructor() {
    this.cb = {
      startSuite: (suite) => {
        this._reportTestSuiteActions('StartSuite', suite);
      },
      finishSuite: (suite) => {
        this._reportTestSuiteActions('FinishSuite', suite);
      },
      startTest: (test) => {
        this._reportTestSuiteActions('StartSuiteTest', test);
      },
      finishTest: (test) => {
        this._reportTestSuiteActions('FinishSuiteTest', test);
      }
    };
  ...
  async _executeStep(fn, suite, test) {
    ...
    try {
      ...
      if ( err.stack )
        test.errorTrace = this._toErrorTrace(err);
    }
  ...
  _reportTestSuiteActions(type, contents) {
    const currentSuite = this.currentSuite;
